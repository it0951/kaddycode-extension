...
import { KaddyCodeLensProvider } from "./codelens/KaddyCodeLensProvider";

export function activate(context: vscode.ExtensionContext) {
  console.log("KaddyCode 활성화됨");
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      const ext = vscode.extensions.getExtension("kaddycode.kaddycode");
      const currentVersion = ext?.packageJSON?.version ?? "";
      if (currentVersion && currentVersion !== data.version) {
        const selected = await vscode.window.showWarningMessage(
          `KaddyCode 업데이트 사용 가능: v${data.version}`,
          "VSIX 다운로드",
          "무시",
        );
  ...
  const codeLensProvider = new KaddyCodeLensProvider();
  ...
  const codeLensActions: Record<
    string,
    {
      ...
      inputPreset?: (lang: string, code: string) => string;
    }
  > = {
    open: {
      label: "KaddyCode",
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand(
        ...
        async (range: vscode.Range) => {
          ...
          // open: 채팅창 포커스만
          if (actionKey === "open") {
            await vscode.commands.executeCommand("kaddycode.chatView.focus");
            return;
          }
          // noop: 아무것도 안 함
          if (actionKey === "noop") {
  ...
  // ── 기존 커맨드 ────────────────────────────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand("kaddycode.chat", async () => {
      await vscode.commands.executeCommand("kaddycode.chatView.focus");
    ...
    vscode.commands.registerCommand(
      "kaddycode.indexCurrentFile",
      indexCurrentFile,
    ),
    vscode.commands.registerCommand("kaddycode.searchCode", searchCode),
    vscode.commands.registerCommand("kaddycode.openChat", () => {
      vscode.commands.executeCommand("kaddycode.chatView.focus");
    }),
    //버전 확인 커맨드
    vscode.commands.registerCommand("kaddycode.checkVersion", async () => {
      const config = vscode.workspace.getConfiguration("kaddycode");
      const serverUrl = config.get<string>("serverUrl", "");
      if (!serverUrl) {
        vscode.window.showWarningMessage(
          "KaddyCode: Internal Server URL이 설정되지 않았습니다.",
        );
        return;
      }
      try {
        ...
        if (currentVersion === serverVersion) {
          vscode.window.showInformationMessage(
            `KaddyCode v${currentVersion} - 최신 버전입니다.`,
          );
        } else {
          const selected = await vscode.window.showWarningMessage(
            `KaddyCode 업데이트 필요: 현재 v${currentVersion} / 서버 v${serverVersion}`,
            "VSIX 다운로드",
          );
          if (selected === "VSIX 다운로드") {
            vscode.env.openExternal(
        ...
        vscode.window.showErrorMessage(
          "KaddyCode: 버전 확인에 실패했습니다. 서버 연결을 확인하세요.",
        );
      }
    }),
    vscode.commands.registerCommand("kaddycode.openSettings", () => {
  ...
  // ── 파일 저장 시 자동 인덱싱 ──────────────────────────────────────────
  const autoIndexDisposable = vscode.workspace.onDidSaveTextDocument(
    async (document) => {
      const config = vscode.workspace.getConfiguration("kaddycode");
      const autoIndex = config.get<boolean>("autoIndexOnSave", false);
      const apiKey = config.get<string>("apiKey", "");

      if (!autoIndex || !apiKey || apiKey.trim() === "") {
        return;
      }
      ...
      try {
        ...
        console.log(`[KaddyCode] 자동 인덱싱 완료: ${document.fileName}`);
      } catch (e) {
        console.warn(`[KaddyCode] 자동 인덱싱 실패: ${e}`);
      }
    },
  );
...
function extractCodeBlock(
  ...
  if (langId === "python") {
    ...
    for (
      let i = startLine + 1;
      i < Math.min(startLine + 150, totalLines);
      i++
    ) {
      ...
      if (getIndent(line) <= startIndent && line.trim() !== "") {
        break;
      }
      lines.push(line);
    }
